class ExpenseTracker {
  ...
  addTransaction(description, amount, type, date, category) {
    ...
    this.showSuccess(
      ...
      } added: ${description} - CFA ${parseFloat(amount).toFixed(2)}`
    );
  ...
  deleteTransaction(id) {
    ...
    this.showInfo(
      `Deleted: ${transaction.description} - CFA ${transaction.amount.toFixed(
        2
      )}`
    );
  ...
  updateUI() {
    ...
    document.getElementById(
      'totalBalance'
    ).textContent = `CFA ${totals.balance.toLocaleString('en-US', {
      minimumFractionDigits: 2,
    })}`;
    document.getElementById(
      'totalIncome'
    ).textContent = `CFA ${totals.income.toLocaleString('en-US', {
      minimumFractionDigits: 2,
    })}`;
    document.getElementById(
      'totalExpense'
    ).textContent = `CFA ${totals.expense.toLocaleString('en-US', {
      minimumFractionDigits: 2,
    })}`;
        ...
        (transaction) => `
            <div class="transaction-item ${transaction.type}">
                ...
                <div class="transaction-amount">
                    ...
                    }CFA ${transaction.amount.toLocaleString('en-US', {
          minimumFractionDigits: 2,
        })}
                </div>
                <div class="transaction-actions">
                    <button class="delete-btn" onclick="expenseTracker.showDeleteModal(${
                      transaction.id
                    })">
    ...
    document
      ...
      .addEventListener('submit', (e) => {
        ...
        if (!description || !amount || amount <= 0) {
          this.showError(
            'Please enter a valid description and amount (amount must be greater than 0)'
          );
          return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    ...
    if (transactionCount === 0) {
      expenseTracker.showInfo(
        ' Welcome! Start by adding your first transaction.',
        6000
      );
    } else {
      expenseTracker.showSuccess(
        `Welcome back! You have ${transactionCount} transaction${
